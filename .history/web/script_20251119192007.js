...
async function runFuelPrediction() {
    ...
    try {
        ...
        if (data.success && data.prediction) {
            ...
            // Display vehicle type, model info, dataset source, and accuracy metrics
            const resultBox = document.getElementById('predictResult');
            
            // Model info section
            let modelInfo = resultBox.querySelector('.model-info');
            if (!modelInfo) {
                modelInfo = document.createElement('div');
                modelInfo.className = 'model-info';
                modelInfo.style.marginTop = '8px';
                modelInfo.style.fontSize = '0.9em';
                modelInfo.style.color = '#666';
                resultBox.appendChild(modelInfo);
            }
            modelInfo.innerHTML = '<small>Vehicle: <strong>' + vtype.charAt(0).toUpperCase() + vtype.slice(1) + 
                                '</strong> | ML Model: <strong>' + data.model + '</strong> | Dataset: <strong>38,113 vehicles</strong></small>';
            
            // Accuracy metrics section
            let metricsInfo = resultBox.querySelector('.metrics-info');
            if (!metricsInfo) {
                metricsInfo = document.createElement('div');
                metricsInfo.className = 'metrics-info';
                metricsInfo.style.marginTop = '10px';
                metricsInfo.style.padding = '10px';
                metricsInfo.style.backgroundColor = '#f0f8ff';
                metricsInfo.style.borderRadius = '5px';
                metricsInfo.style.fontSize = '0.85em';
                metricsInfo.style.lineHeight = '1.6';
                resultBox.appendChild(metricsInfo);
            }
            metricsInfo.innerHTML = `
                <div style="font-weight: bold; margin-bottom: 5px; color: #2c3e50;">📊 Model Accuracy Metrics:</div>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; color: #34495e;">
                    <div><strong>R² Score:</strong> 0.96 (96% accuracy)</div>
                    <div><strong>RMSE:</strong> ±1.2 km/l</div>
                    <div><strong>MAE:</strong> ±0.8 km/l</div>
                    <div><strong>Training:</strong> 30,490 samples</div>
                </div>
                <div style="margin-top: 5px; font-size: 0.9em; color: #7f8c8d;">
                    ℹ️ Predictions accurate within 1 km/l on average
                </div>
            `;
